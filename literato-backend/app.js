...
app.get('/dbsearch/:title', (req, res) => {
  ...
  Book.sequelize
    .query(
      'SELECT "Books"."userId","Books"."title","Books"."authors","Books"."description", "Books"."id", "Users"."username" FROM "Books" LEFT OUTER JOIN "Users" ON "Users"."id" = "Books"."userId" WHERE "Books"."title" = :title',
...
app.post('/user', (req, res) => {
  ...
  req.getValidationResult().then(validationErrors => {
    if (validationErrors.isEmpty()) {
      User.find({ where: { authToken: req.body.authToken } })
        .then(user => {
          if (user.authExpired()) {
            res.status(400)
